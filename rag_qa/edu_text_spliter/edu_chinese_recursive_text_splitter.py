...
if __name__ == "__main__":
    ...
    for inum, text in enumerate(doc):
        ...
        print(f"第{inum + 1}个文档； 总分块数量 [{len(chunks)}]")
        print("-" * 30)
        for chunk in chunks:
            print(f"第{chunks.index(chunk) + 1}块；分块大小 [{len(chunk)}] : {chunk}")
        print("-" * 30)
# 每块没有重叠，是因为每个分块分割的时候使用了.strip()去除了首位空格和换行符等影响了重叠的字符计算； 适当改变chunk_size和chunk_overlap参数，可以有重叠效果。

    print("CharacterTextSplitter === 普通字符分割器 ===")
    for inum, text in enumerate(doc):
        chunks = char_splitter.split_text(text)
        for chunk in chunks:
            print(f"分块大小 [{len(chunk)}] : {chunk}")
